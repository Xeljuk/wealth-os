...
export default function IncomeFormModal({ mode, initial, onClose, onSubmit }: IncomeFormModalProps) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSubmitting(true);
    try {
      await onSubmit({ name: name.trim(), amount: a, recurring });
    } catch (err) {
      ...
      >
        <div className="flex items-start justify-between">
          <div>
            ...
            >
              {mode === "create" ? "Add an income source" : initial?.name}
            </h2>
          </div>
          <button
            type="button"
        ...
        <form className="mt-5 flex flex-col gap-4" onSubmit={handleSubmit}>
          ...
          <div className="mt-2 flex items-center justify-end gap-2">
            ...
            >
              ...
              {mode === "create" ? "Create Income" : "Save Changes"}
            </button>
          </div>
        </form>
      </div>
